...
void writeBMP(int width, int height);
// settings
...
int main()
{
	...
	// gl::Shader debug_shadow(gl::ShaderType::VertexShader, "../data/shaders/debug_shadow.vert");
	...
	// gl::Shader light_debug_shadow(gl::ShaderType::FragmentShader, "../data/shaders/light_debug_shadow.frag");

	gl::Program light_shadow_program(&p3t2n3_vs, &light_shadow_fs);
	// gl::Program shadow_program(&debug_shadow, &light_debug_shadow);
	gl::Program shadow_program(&p3_vs, &empty_fs);
	pointf3 point_light_pos = { 3,2,2 };
	rgbf ambient{ 0.2f,0.2f,0.2f };
	light_shadow_program.SetTex("albedo_texture", 0);
	light_shadow_program.SetTex("shadowmap", 1);
	light_shadow_program.SetVecf3("point_light_pos", point_light_pos);
	...
	// -------------------------
	gl::Texture2D spot_albedo = loadTexture("../data/textures/spot_albedo.png");//0 tex
	gl::TextureCubeMap cube;
	// shadow buffer
	// -------------------------
	const size_t SHADOW_TEXTURE_SIZE = 1024;
	gl::Texture2D shadowmap;//1 tex
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// gl::ReadPixels(0, 0, SHADOW_TEXTURE_SIZE, SHADOW_TEXTURE_SIZE, GL_DEPTH_COMPONENT, );
		...
		transform<float> light_projection = transformf::orthographic(20.0f, 20.0f, near_plane, far_plane);
		transform<float> light_view = transformf::look_at(point_light_pos, pointf3{ 0,0,0 }, vecf3{ 0,1,0 });
		transform<float> light_model(point_light_pos);
		shadow_program.SetMatf4("projection", light_projection);
		shadow_program.SetMatf4("view", light_view);
		shadow_program.SetMatf4("model", light_model);

		shadow_program.SetTex("depthMap", 1);
		shadow_program.SetFloat("near_plane", near_plane);
		shadow_program.SetFloat("far_plane", far_plane);
		// glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		// glEnable(GL_DEPTH_TEST);
		spot->va->Draw(&shadow_program);
		//=================write_shadow_map in PNG================
		//=================================
		// writeBMP(SHADOW_TEXTURE_SIZE, SHADOW_TEXTURE_SIZE);
		...
		light_shadow_program.SetMatf4("light_space_mat", light_projection * light_view);
...
void writeBMP(int width, int height)
{
	...
	// 将深度数据保存为图片文件
	FILE* file = fopen("../data/depth.bmp", "wb");
	if (file)
	{
		// BMP 文件头
		unsigned char header[54] = {
			'B', 'M', 0, 0, 0, 0, 0, 0, 0, 0, 54, 0, 0, 0,
			40, 0, 0, 0, 0, 2, 0, 0, 224, 1, 0, 0,
			1, 0, 24, 0, 0, 0, 0, 0,
			(unsigned char)(width * height * -3), (unsigned char)((width * height * -3) >> 8), (unsigned char)((width * height * -3) >> 16), (unsigned char)((width * height * -3) >> 24),
			(unsigned char)(11810), (unsigned char)(11810 >> 8), (unsigned char)(11810 >> 16), (unsigned char)(11810 >> 24),
			(unsigned char)(11810), (unsigned char)(11810 >> 8), (unsigned char)(11810 >> 16), (unsigned char)(11810 >> 24),
			0x00
		};
		...
		for (int y = height - 1; y >= 0; y--)
		{
			for (int x = 0; x < width; x++)
			{
				...
				unsigned char color = (unsigned char)(depth * 255);
				fputc(color, file);
				fputc(color, file);
				fputc(color, file);
			}
