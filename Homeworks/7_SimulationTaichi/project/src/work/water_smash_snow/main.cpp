...
void add_jet() {
    add_object_rectangle(Vec(0.5, 0.05), Vec(0.51, 0.06), 0x87CEFA, 10, Vec(0.0, 25.0));
...
/***********************************(5)*****************************************/
  ...
  add_object(Vec(0.5,0.2), 0xFFFAFA, 2);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for (int i = 0;; i++) {                              //              Main Loop
    ...
    if (i % int(frame_dt / dt) == 0) {                 //        Visualize frame
      ...
      if (i < 5e3) add_jet();
      canvas.img.write_as_image(fmt::format("D:\\fuxian\\MyCG\\USTC_CG\\Homeworks\\7_SimulationTaichi\\project\\src\\work\\water_smash_snow\\output\\{:05d}.png", f++));
    }
  }
} //----------------------------------------------------------------------------
