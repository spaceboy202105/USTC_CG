...
auto hardening = 10.0_f, E = 1e4_f, nu =0.1_f;
...
int main() {
	...
	add_object_circle(Vec(0.5, 0.5), 0.08, 0xFFFAFA, 1200);
	////////////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0;; i++) {                              //              Main Loop
		...
		if (i % int(frame_dt / dt) == 0) {                 //        Visualize frame
			...
			// auto file = fopen(fmt::format("tmp\\test.png").c_str(), "wb");
			// if (!file)return -1;
			canvas.img.write_as_image(fmt::format("D:\\fuxian\\MyCG\\USTC_CG"
										 "\\Homeworks\\7_SimulationTaichi\\project\\src\\work"
		   "\\snow_with_E\\output\\nu_0_1_{:05d}.png", f++));
		}
	}
} //----------------------------------------------------------------------------
