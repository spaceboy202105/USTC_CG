...
void add_jet() {
	add_object_rectangle(Vec(0.5, 0.05), Vec(0.51, 0.06), color_rand(), 10, Vec(0.0, 17.0));
	...
	for (int i = 0;; i++) {                              //              Main Loop
		...
		if (i % int(frame_dt / dt) == 0) {                 //        Visualize frame
			...
			canvas.img.write_as_image(fmt::format("D:\\fuxian\\MyCG\\USTC_CG\\Homeworks\\7_SimulationTaichi\\"
				"project\\src\\work\\color_fountain\\tmp\\{:05d}.png", f++));
			/***********************************(2)*****************************************/
			if (i < 5e3) add_jet();//control total time
	  ////////////////////////////////////////////////////////////////////////////////
		}
	}
} //----------------------------------------------------------------------------
