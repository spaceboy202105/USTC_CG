...
int main() {
	...
	add_object(Vec(0.15, 0.85), 0x024517);
	...
	for (int step = 0;; step++) {
		...
		if (step % int(frame_dt / dt) == 0) {
			...
			// Update image
			gui.update();
			auto f = fopen("D:\\fuxian\\MyCG\\USTC_CG\\Homeworks\\7_SimulationTaichi\\project\\src\\work\\color_fountain\\tmp\\test.png", "wb");

			// Write to disk (optional)
			canvas.img.write_as_image(fmt::format("D:\\fuxian\\MyCG\\USTC_CG\\Homeworks\\7_SimulationTaichi\\"
				"project\\src\\work\\color_fountain\\tmp\\{:05d}.png", frame++));
			// canvas.img.write_to_disk(fmt::format("tmp/{:05d}.png", frame++));
		}
	}
}
